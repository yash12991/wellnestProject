...
class SmartMealReplacementService {
  ...
  async _generateWithRetries(prompt, modelNames = [ 'gemini-2.5-flash', 'gemini-2.5', 'gemini-2.0'], maxRetries = 3) {
  ...
  const response = await this._generateWithRetries(enhancedPrompt, [ 'gemini-2.5-flash', 'gemini-2.5']);
  ...
  const response = await this._generateWithRetries(prompt, [  'gemini-2.5-flash', 'gemini-2.5']);
  const parsed = this._parseAIJSON(response);
      if (parsed) return parsed;
